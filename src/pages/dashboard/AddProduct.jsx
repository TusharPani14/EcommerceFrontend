  ...
  const handleImport = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_SERVER_URL}/product/importCSV`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      {loading ? (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            {/* Input field hidden by default, trigger click event when the button is clicked */}
            <label className="flex items-center bg-[#FF7004] px-4 py-2.5 my-1 min-w-[150px] font-medium text-[11.2px] md:text-[13px] text-white cursor-pointer">
              <span className="mr-1">Import CSV</span>
              <svg
